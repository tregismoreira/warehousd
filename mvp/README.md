...
<p align="center">
  <strong>An MCP-ready governed data layer — your company's data, safely queryable by AI assistants.</strong>
</p>
...
| SSO — OIDC and SAML | **real** | Better Auth SSO plugin, e2e-tested against Keycloak. The manual connector-plus-real-IdP runbook is written but has not been executed yet. |
...
| `warehousd deploy` (Fly.io) | *not built* | Planned; deploy by running the image yourself for now. |
...
Contributing to warehousd itself (consumers never need this):
...
To run the app against the Meridian demo company — three personas, six
collections, planted canary values, and a pre-seeded grant request — see
[SETUP.md](../docs/SETUP.md). Demo mode seeds `ana@demo.local` (admin),
`marcus@demo.local` (manager), and `mia@demo.local` (member), password `demo`.
...
Further reading: [SPECS.md](../docs/SPECS.md) (design spec and acceptance
tests) · [GLOSSARY.md](../docs/GLOSSARY.md) (canonical terminology) ·
[MVP-ROADMAP.md](../docs/MVP-ROADMAP.md) (phases and status) ·
...
aggregate-only postures with inference-leak protection · additional deploy
targets · IdP group→role mapping.
