...
- **Keyset pagination.** `after`/`nextCursor` on `POST /v1/collections/{c}/query` walk a collection by a stable `(sortField, pk)` cursor instead of an offset, so a page fetched while the collection is being written to concurrently neither skips nor repeats a document — the failure mode offset pagination has under a concurrent insert or approval. The cursor is opaque but not signed: a forged one only ever produces a comparison the grant's document filters, per-document ACL and postures already apply to every other query, so it cannot read anything an ordinary filter could not. Requires a declared primary key and a non-nullable sort field, and refuses `field_denied` if the primary key falls outside the caller's `allowedFields` — the cursor carries its value back on every page. Documented in [docs/rest-api.md](docs/rest-api.md#pagination).
