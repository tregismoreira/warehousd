...
import { upsertClientPolicy, approveGrant, requestGrant, revokeGrant, setAllowedScopes } from "@warehousd/broker";
...
async function issueTokenWithLiveScope(collectionSuffix: string = "") {
  ...
  const reg = await db.auth.api.registerMcpClient({
    body: { redirect_uris: ["http://localhost:9999/callback"], client_name: `Refresh Test Client${collectionSuffix}` },
    asResponse: true,
  } as any);
  const { client_id, client_secret } = await reg.json(); // snake_case — RFC 7591
  await upsertClientPolicy(app, client_id, `Refresh Test Client${collectionSuffix}`, ["env:dev", "env:live"]);
  ...
  const { code } = await authorizeAndGetCode(db.auth, {
    clientId: client_id, scope: "env:live offline_access", cookie: miaCookie, challenge,
  });
