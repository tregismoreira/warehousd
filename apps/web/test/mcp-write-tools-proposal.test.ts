...
const ctx = { userId: "mia", orgId: "default", env: "dev" as const, via: "session" };
...
describe("mcp-tools: create_document under a proposal_only grant", () => {
  ...
  beforeAll(async () => {
    ...
    const grantId = await requestGrant(app, {
      ...
      purposeLabel: "test", allowedFields: ["id", "title", "message", "submitted_by", "created_at"],
    });
  ...
  it("yields status: pending, invisible until a human approves", async () => {
    ...
    const out = await tool.handler(ctx, {
      collection: "feedback", values: { title: "t", message: "m", submitted_by: "mia" },
