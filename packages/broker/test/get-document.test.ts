...
import { createAppSchema, applyConfig, createPools, makeBroker, withOrg } from "../src/index";
import { requestGrant, approveGrant } from "../src/grants/manage";
import type { BrokerContext, GetDocumentIntent } from "../src/types";
...
const ctx: BrokerContext = { userId: "user1", env: "live", orgId: "default" };
...
describe("broker.getDocument", () => {
  ...
  it("a document excluded by the document filter is not_found", async () => {
    ...
    const grantId = await requestGrant(app, {
      userId, collection: "people", env: "live", orgId: "default",
      purposeLabel: "test", allowedFields: ["id", "email", "status"],
    });
  ...
  it("$self filter scopes it", async () => {
    ...
    const ctxSelf: BrokerContext = { userId, env: "live", orgId: "default" };

    // Request grant with $self filter
    const grantId = await requestGrant(app, {
      userId, collection: "people", env: ctxSelf.env, orgId: ctxSelf.orgId,
    ...
    const ownedResult = await broker.getDocument(ctxSelf, { collection: "people", id: ownedId.toString() });
    expect(ownedResult.ok).toBe(true);

    // Should not find other's document
    const otherResult = await broker.getDocument(ctxSelf, { collection: "people", id: otherId.toString() });
  ...
  it("a grant without read verb refuses with no_grant", async () => {
    ...
    const ctxNoRead: BrokerContext = { userId: "noread_user", env: "live", orgId: "default" };
    const id = (await app.query(
  ...
  it("path form works on a file collection and is invalid_intent on a dataset", async () => {
    ...
    const ctxFile: BrokerContext = { userId: fileUser, env: "live", orgId: "default" };
    const fileGrantId = await requestGrant(app, {
      userId: fileUser, collection: "sources", env: "live", orgId: "default",
      purposeLabel: "test", allowedFields: ["path", "content"],
    });
    ...
    const pathResult = await broker.getDocument(ctxFile, { collection: "sources", path: "test.md" });
    expect(pathResult.ok).toBe(true);

    // Path form on dataset should fail
    const datasetUser = "user_path_dataset_test";
    const ctxPath: BrokerContext = { userId: datasetUser, env: "live", orgId: "default" };
    const datasetGrant = await requestGrant(app, {
      userId: datasetUser, collection: "people", env: "live", orgId: "default",
  ...
  it("a file's chunks come back as one document, not the first chunk", async () => {
    ...
    const ctxTest: BrokerContext = { userId, env: "live", orgId: "default" };
    const grantId = await requestGrant(app, {
      userId, collection: "sources", env: "live", orgId: "default",
    ...
    const r = await makeBroker(pools, cfg).getDocument(ctxTest, { collection: "sources", path: "multi.md" });
  ...
  it("every outcome writes an audit row", async () => {
    ...
    // Allowed outcome
    const successResult = await broker.getDocument(ctxTest, { collection: "people", id: id.toString() });
    expect(successResult.ok).toBe(true);
    if (successResult.ok) {
      const auditRow = await app.query(`select * from app.audit_events where id = $1`, [successResult.auditId]);
      expect(auditRow.rowCount).toBe(1);
      expect(auditRow.rows[0].outcome).toBe("allowed");
    }

    // Refused outcome (no_grant)
    const refusedResult = await broker.getDocument(
      { userId: "no_access", env: "live", orgId: "default" },
      { collection: "people", id: id.toString() }
    );
