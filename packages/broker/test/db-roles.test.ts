...
it("test 5 (partial): dev token cannot see live-only canary; direct role check", async () => {
  ...
  const r = await broker.query(makeCtx({ userId: "u" }), { collection: "people", limit: 500 });
...
it("test 5 (scope clauses): full env-as-scope acceptance gate", async () => {
  ...
  try {
    ...
    await approveGrant(app, cfg, grantId, "marcus", {
      expiresAt: new Date(Date.now() + 86_400_000).toISOString(),
    });
    const refreshed = await web.auth.api
      .mcpOAuthToken({
        body: {
    ...
    const demoted = await web.auth.api
      .mcpOAuthToken({
        body: {
          grant_type: "refresh_token",
    ...
    const ctx = await deriveTokenContext(
      new Request("http://localhost:8722/mcp", {
    ...
    const row = await app.query(`select * from app."oauthAccessToken" where "accessToken"=$1`, [
      noEnv.access_token,
    ]);
    const cols = Object.keys(row.rows[0]);
    expect(cols).not.toEqual(
      expect.arrayContaining(["allowedFields", "purposeLabel", "documentFilter"]),
    );
    ...
    const g2 = await app.query(
      `select id from app.grants where user_id='mia' and env='live' and status='approved' order by requested_at desc limit 1`,
    );
    await revokeGrant(app, g2.rows[0].id, "marcus");
    const afterRevoke = await web.auth.api
      .mcpOAuthToken({
        body: {
          grant_type: "refresh_token",
...
describe("env role grants (design §8 test 7)", () => {
  ...
  it("env role reads file view but not base tables", async () => {
    ...
    await db.query(
      ...
      values (gen_random_uuid(),$1,0,'policy content')`,
      [d.rows[0].id],
    );
    ...
    await expect(dev.query(`select * from data_synth."policies__files"`)).rejects.toThrow(
      /permission denied/,
    );
    await expect(dev.query(`select * from data_synth."policies__documents"`)).rejects.toThrow(
      /permission denied/,
    );
    await expect(dev.query(`select * from data_live.v_policies`)).rejects.toThrow(
      /permission denied/,
    );
    await dev.end();
    await db.end();
