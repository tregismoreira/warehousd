...
let p: Provisioned;
beforeAll(async () => { p = await provision("pool-timeouts"); }, 60_000);
afterAll(async () => { await p?.end(); });

const urlsOf = (pr: Provisioned) => ({
  app: pr.urls.admin, dev: pr.urls.dev, live: pr.urls.live,
  imp: pr.urls.imp, devWrite: pr.urls.devWrite, liveWrite: pr.urls.liveWrite,
});
...
describe("pool statement bounds", () => {
  ...
  it("cancels a statement that overruns the bound, inside withOrg's transaction", async () => {
    ...
    try {
      ...
      await expect(withOrg(pools.dev, "default", (c) => c.query(`select pg_sleep(10)`)))
  ...
  it("gives the app and import pools a longer bound than the query pools", async () => {
    ...
    const pools = createPools(urlsOf(p));
    const ms = async (pool: import("pg").Pool) => Number((await pool.query(
      `select setting::bigint as v from pg_settings where name='statement_timeout'`)).rows[0].v);
    try {
  ...
  it("ignores an unparseable override rather than dropping the bound", async () => {
    ...
    const pools = createPools(urlsOf(p));
    try {
      expect((await pools.dev.query(`show statement_timeout`)).rows[0].statement_timeout).toBe("30s");
    } finally {
      await pools.end();
      delete process.env.WAREHOUSD_STATEMENT_TIMEOUT_MS;
    }
