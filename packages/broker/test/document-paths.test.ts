...
beforeAll(async () => {
  ...
  // A second tenant's file, written straight in: indexCollection has no org parameter, and what
  // this file has to prove is that a *reader* scoped to one org never sees the other's paths.
...
describe("listDocumentPaths", () => {
  ...
  // The org used to be pinned to DEFAULT_ORG_ID inside this function, so an approver in any other
  // tenant was shown the default tenant's filenames and offered them as a grant scope.
  it("scopes to the caller's org — neither tenant sees the other's paths", async () => {
    const mine = await listDocumentPaths(pools, dev, cfg, "policies");
    const theirs = await listDocumentPaths(
      pools,
      { env: "dev", orgId: OTHER_ORG },
      cfg,
      "policies",
    );
