...
const cfg: WarehousdConfig = {
  ...
  taxonomies: {}, demo: false,
  collections: { people: { description: "dir", type: "dataset", fields: {
    id: { type: "uuid", posture: "allow", pk: true },
    full_name: { type: "text", posture: "allow" },
  }}},
} as unknown as WarehousdConfig;
...
beforeAll(async () => {
  ...
  await admin.query(
    `insert into data_synth.people (org_id, id, full_name) values
       ($1, gen_random_uuid(), 'Ana of A'), ($2, gen_random_uuid(), 'Bo of B')`, [ORG_A, ORG_B]);
  for (const org of [ORG_A, ORG_B])
    await admin.query(
      `insert into app.grants (org_id,user_id,collection,allowed_fields,env,status)
       values ($1,'shared','people', array['id','full_name'],'dev','approved')`, [org]);
...
describe("org isolation", () => {
  it("a query in org A returns only org A's documents", async () => {
    const r = await broker.query({ userId: "shared", orgId: ORG_A, env: "dev" }, { collection: "people" });
  ...
  it("the same user in org B sees only org B's documents", async () => {
    const r = await broker.query({ userId: "shared", orgId: ORG_B, env: "dev" }, { collection: "people" });
    expect(r.ok).toBe(true);
  ...
  it("the database is what refuses — the broker's SQL carries no org predicate", async () => {
    const { text, values } = buildSelect("dev",
      { collection: "people", fields: ["id", "full_name"] }, ["id", "full_name"]);
  ...
  it("RLS is enabled with an org_isolation policy on every data table", async () => {
    ...
    expect(r.rows.map((x) => `${x.schemaname}.${x.tablename}`))
      .toEqual(["data_live.people", "data_synth.people"]);
