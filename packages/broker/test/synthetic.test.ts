...
const cfg: WarehousdConfig = ConfigSchema.parse({
  project: "t",
  server: { port: 1 },
  synthetic: { documents_per_collection: { people: 10, salaries: 20 } },
  collections: {
    people: {
      description: "dir",
      fields: {
...
describe("synthetic: self-referential and cyclic FKs", () => {
  ...
  it("cyclic FK gets backfilled with non-null values", async () => {
    ...
    const deptHeads = (
      await db.query(
        `select count(*)::int as n from data_synth.departments where head_id is not null`,
      )
    ).rows[0].n;
    const peopleDepts = (
      await db.query(
        `select count(*)::int as n from data_synth.people where department_id is not null`,
      )
    ).rows[0].n;
