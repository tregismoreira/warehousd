...
it("lists names + descriptions only, even with zero grants", async () => {
  ...
  const r = await broker.listCollections({ userId: "nobody", orgId: "default", env: "dev", via: "session" });
  expect(r).toEqual([
...
it("writes an audit row with collection='*' and outcome='allowed'", async () => {
  const broker = makeBroker(pools, cfg);
  await broker.listCollections({ userId: "alice", orgId: "default", env: "live", via: "session" });
