...
describe("app schema", () => {
  it("creates collections, grants, audit_events tables", async () => {
    ...
    await db.end();
    expect(r.rows.map((x) => x.table_name)).toEqual(["audit_events", "change_log", "client_policies", "client_secrets", "collections", "grants", "organizations", "terms", "trusted_issuers", "vocabularies"]);
  });
