...
export function declaredTables(collection: string, cfg: WarehousdConfig): DeclaredTable[] {
  ...
  return kindOf(c).ddl.declaredTables(collection, cfg);
}

/** The `type: file` half of declaredTables: two tables, one of them all-structural. */
export function fileDeclaredTables(collection: string, cfg: WarehousdConfig): DeclaredTable[] {
  const c = cfg.collections[collection];
  if (!c) throw new Error(`Unknown collection: ${collection}`);
  ...
  return [
    {
      ...
      structural: [
        ...
        "byte_size",
        "blob",
        // How the row arrived, and the hash of the bytes as stored. The second is generated by
        // Postgres rather than written by us, so an upload plan can ask "do you already have
        // these exact bytes?" without reading a single blob back out.
        "origin",
        "blob_checksum",
      ],
    },
...
/** The `type: dataset` half of declaredTables: one table carrying the declared fields. */
export function datasetDeclaredTables(collection: string, cfg: WarehousdConfig): DeclaredTable[] {
  const c = cfg.collections[collection];
  if (!c) throw new Error(`Unknown collection: ${collection}`);
...
// The shared preamble, then the kind's own half. Nothing here branches on a kind id — the
// registry in config/kinds/ picks, and a fourth kind is one entry there. See kinds/types.ts.
export function tableDDL(env: "dev" | "live", collection: string, cfg: WarehousdConfig): string {
  ...
  return kindOf(c).ddl.table(env, collection, cfg);
}

/** The `type: file` half of tableDDL. Registered by kinds/file.ts; never called directly. */
export function fileTableDDL(
  env: "dev" | "live",
  collection: string,
  cfg: WarehousdConfig,
): string {
...
/** The `type: dataset` half of tableDDL. Registered by kinds/dataset.ts. */
export function datasetTableDDL(
  env: "dev" | "live",
  collection: string,
  cfg: WarehousdConfig,
): string {
...
export function viewDDL(env: "dev" | "live", collection: string, cfg: WarehousdConfig): string {
  const c = cfg.collections[collection];
  if (!c) throw new Error(`Unknown collection: ${collection}`);
  return kindOf(c).ddl.view(env, collection, cfg);
}

/** The `type: file` half of viewDDL. Registered by kinds/file.ts. */
export function fileViewDDL(env: "dev" | "live", collection: string, cfg: WarehousdConfig): string {
  const schema = env === "dev" ? "data_synth" : "data_live";
  const c = cfg.collections[collection];
  if (!c) throw new Error(`Unknown collection: ${collection}`);
...
export function datasetViewDDL(
  env: "dev" | "live",
  collection: string,
  cfg: WarehousdConfig,
): string {
...
export function rlsDDL(env: "dev" | "live", collection: string, cfg: WarehousdConfig): string {
  ...
  // identical on each, so the kind decides the list and nothing else here knows the difference.
  return kindOf(c)
    .ddl.rlsTables(schema, collection)
    .map(
      (t: string) => `
    alter table ${t} enable row level security;
    drop policy if exists org_isolation on ${t};
...
export function grantImportDDL(collection: string, cfg: WarehousdConfig): string {
  ...
  if (c.source_ref) return "";
  return kindOf(c).ddl.grantImport(collection, cfg);
}

/** File collections are populated by the indexer under the owner role, not by import. */
export function fileGrantImportDDL(): string {
  return "";
}

export function datasetGrantImportDDL(collection: string): string {
  return `
...
export function grantWriteDDL(
  ...
  return kindOf(c).ddl.grantWrite(schema, role, collection);
}

export function fileGrantWriteDDL(schema: string, role: string, collection: string): string {
...
export function datasetGrantWriteDDL(schema: string, role: string, collection: string): string {
  return `
