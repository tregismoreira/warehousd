...
The `<suffix>` is a hash of the checkout path. Databases are cluster-global and sibling checkouts share this Postgres, so without it one checkout's globalSetup would drop the template another one is mid-run cloning from — the same destructive failure described under "Running two checkouts at once" below.
...
`pnpm test` runs in two passes: `test:parallel` (every file, four workers) then `test:serial`. `WAREHOUSD_TEST_WORKERS` changes the worker count — it defaults to 4 because sibling checkouts share this machine.
...
Sibling checkouts share `127.0.0.1:54330`, whichever one ran `pnpm test:up` first. Sharing the Postgres _server_ is fine and intended. Sharing a _database_ or an _app port_ is not:

- A shared database is destructive — each `e2e:setup` drops and recreates it, so one suite pulls the schema out from under the other mid-run. It surfaced as `relation "session" does not exist`, `column g.workspace_id does not exist`, and hangs well past Playwright's own timeout.
...
- **Databases** — `scripts/e2e-setup.ts` and `apps/web/playwright.config.ts` independently derive `warehousd_e2e_<checkout-dir>`. Override with `WAREHOUSD_E2E_DB`.
...
- **Adoption** — `reuseExistingServer` is `false` unconditionally. There is nothing legitimate to reuse once the port is per-checkout, and the failure it buys back is a loud one.
...
- **Tenant isolation, data plane** (`workspace-isolation`) — two workspaces' documents in one collection; each workspace's query returns only its own. The proof that _the database_ is what refuses: the SQL `buildSelect` produced is asserted to contain no `workspace_id`, then run directly against the view under each workspace, and it still separates the rows. With no workspace in scope the view returns nothing — the wall fails closed.
- **Tenant isolation, control plane** (`workspace-control-plane`) — each decision function carries the workspace predicate itself: a manager cannot approve, deny or revoke another workspace's grant, an omitted workspace fails closed rather than open, and `env:live` eligibility does not leak across workspaces for the same user id.
- **Control-plane RLS, all eight tables** (`apps/web/test/control-plane-isolation.integration.test.ts`) — for each of `app.grants`, `app.audit_events`, `app.client_policies`, `app.client_secrets`, `app.trusted_issuers`, `app.user_groups`, `app.change_log` and `app.workspace_members`: a row seeded in workspace A and one in B, and every listing route returns only the active workspace's — asserted per table, not once in aggregate. A revert-check removes one route's own predicate and confirms its assertion fails despite RLS being enabled, proving the predicate — not the policy — is what the route depends on; the owner pool every control-plane route connects through is exempt from RLS by Postgres's own rule for a table's owner.
- **Multi-membership and per-workspace role** (`apps/web/test/workspace-members.integration.test.ts`) — `requireRole` resolves against membership in the session's *active* workspace, not a global role column; a user with no membership in the target workspace gets 403, not 401; `setMember`/`removeMember` upsert and refuse to strand a workspace's last admin (`LastAdminRemoval`).
- **Workspace resolution at the auth boundary** (`apps/web/test/workspace-resolution.test.ts`, `apps/web/test/workspace-switch.integration.test.ts`) — `resolveWorkspace` intersects a requested workspace against loaded membership rather than trusting it; naming a non-membership refuses and changes nothing; `GET`/`POST /api/me/workspace` return exactly the caller's own memberships and switch the session's active workspace only among them.
- **Console membership UI, gated by the flag** (`apps/web/test/admin-members.integration.test.ts`) — 404s on every method with `workspaces.enabled: false`; a manager is refused; role changes and removals enforce the same last-admin guard as the broker module, reached through the console route.
- **The flag gates surface, never enforcement** (`packages/broker/test/workspace-toggle.integration.test.ts`) — the identical enforcement assertions run twice, `workspaces.enabled: true` and `false`: cross-workspace reads are still refused, RLS still rejects an unscoped write, and audit rows still carry `workspace_id`, whichever way the flag is set.
- **Taxonomy terms do not cross workspaces** (`packages/broker/test/workspace-terms.integration.test.ts`) — a canary planted in workspace A's dataset-sourced vocabulary appears in A's term bindings and zero times in the serialized JSON of every B response (bindings, term picker, `describeCollection`); config-declared terms remain visible in both.
- **Seeding and ingest land in the named workspace** (`packages/broker/test/workspace-seeding.integration.test.ts`) — `generateSynthetic`, `regenerateSynthetic`, `ingestFile` and `indexCollection` each scoped to a workspace; regenerating A leaves B's rows untouched (the test that would have caught `regenerateSynthetic`'s former cross-tenant `truncate`); a raw insert with no `workspace_id` and no GUC set is rejected by RLS rather than defaulted.
- **`embedCollection` does not cross workspaces** (`packages/broker/test/embed-workspace-scope.integration.test.ts`) — embedding workspace A's pending chunks leaves B's untouched, closing a gap where a scan with no workspace filter let one workspace's admin consume another's provider quota.
- **Per-document ACL management does not cross workspaces** (`packages/broker/test/workspace-acl.integration.test.ts`) — an admin of A cannot get or set a document ACL scoped to B; a `can_manage_acl` client policy set in A does not carry the flag into B.
- **The platform control plane** (`apps/web/test/platform-api.integration.test.ts`, `packages/cli/test/platform-key.test.ts`) — every `/v1/platform/*` route 404s with the flag off and requires a valid platform key with it on; a key scoped to specific workspaces gets `404 not_found`, never `403`, naming a workspace outside its `managed_workspaces`; `platform-key create/list/revoke` mint a hashed credential, never round-trip the secret after creation, and refuse the same way the routes do when the flag is off.
- **Platform key internals** (`packages/broker/test/platform-keys.test.ts`) — `listPlatformKeys` never returns the secret hash; a key cannot be minted with an expiry past `MAX_KEY_LIFETIME_DAYS`; `mayManageWorkspace` treats a `null` ACL as every workspace and a populated one as only its members.
- **The console switcher and per-workspace data** (`apps/web/e2e/workspace-switch.spec.ts`) — a member of more than one workspace sees the switcher and switching changes the visible collection counts; a member of exactly one sees the active workspace's name with no switcher control.
...
- **Reading and diffing a revision** (`revision-read`) — a past revision is projected through the caller's current grant: a denied field is absent from the document and from every log line, a masked field is masked in the statement, and a document the grant's filter or the document's ACL excludes refuses `not_found`.
- **Immutability by privilege** (`write-privileges`, against real Postgres) — the write role _cannot_ UPDATE a data column and _cannot_ DELETE, asserted as Postgres errors and again against `information_schema`; it _can_ insert and update `_current`/`_rev_status`; RLS confines its base-table SELECT to one workspace.
- **Full-document reads** (`get-document`) — only granted fields come back; a document the filter excludes is `not_found`, the same answer as one that does not exist; `$self` scopes it; `path` on a dataset is `invalid_intent`; a file's chunks are rejoined into one document rather than returning the first chunk; `workspace_id` and `_rev*` never appear; every outcome writes an audit row.
...
- **Change feed** (`change-feed`) — a create, update, delete, file create, proposal and approval each write exactly one entry; the feed carries no field values and no field names, asserted by stringifying and grepping; `since` is exclusive and strictly ordered; a caller sees only their own workspace's and env's entries, and none from a collection they hold no grant on. Two proofs worth naming: revoking the write role's `insert` on `app.change_log` makes the whole mutation disappear, showing the revision and the feed row share one transaction; and two interleaved writers yield every committed revision exactly once across successive polls, showing the cursor is not lossy when `seq` order diverges from commit order.
...
- **Inventory reads stay inside the tenant and the environment** (`documents-inventory`) — counts, file listings and term usage all read through the env-scoped pool inside `withWorkspace`, so one workspace's documents never enter another's totals and a collection with no view in this environment is reported as absent rather than aborting the transaction that was counting its neighbours.
- **Declared indexes** (`indexes.test.ts`) — a declared index reaches the database with `workspace_id` leading and the declared primary key trailing, in both environments, and every dataset carries the structural history index.
- **To-one relations** (`relation-read.test.ts`) — a relation exposes only the target fields the host declared, masks the ones it declared masked, resolves to null across a workspace boundary or an ACL the caller is not a principal of, and yields one host document however many revisions the target has.
