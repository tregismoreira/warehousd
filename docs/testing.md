...
- **Declared indexes** (`indexes`) — a declared index reaches the database with `workspace_id` leading and the declared primary key trailing, in both environments, and every dataset carries the structural history index.
- **To-one relations** (`relation-read`) — a relation exposes only the target fields the host declared, masks the ones it declared masked, resolves to null across a workspace boundary or an ACL the caller is not a principal of, and yields one host document however many revisions the target has.
