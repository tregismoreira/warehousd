...
- `init`, `start` and `restart` open with the wordmark on stderr, above the progress. It is drawn **only** on a terminal: piped, redirected, in CI, under `--quiet` or `--json`, nothing at all is written — not even a plain-text version — so `warehousd start 2>/dev/null` still prints the summary alone and a CI log carries no decoration. It shrinks to one line on a narrow terminal and uses the brand's own green where the terminal reports 24-bit or 256-colour support, falling back to plain ANSI green otherwise. `NO_COLOR`, `TERM=dumb` and `--no-color` remove the colour as they do everywhere else.
