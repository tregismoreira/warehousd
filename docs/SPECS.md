...
| **Grant** | (user, collection, purpose, allowed-fields ⊆ collection allow-list, env, expires_at). Grants are requested by users, approved by a manager/admin, and evaluated **at query time** (revocation is immediate — never baked into tokens). |
...
**Acceptance subset (gates Phase 0):** tests 1, 2, 3, 4, 7, 8, 9, 10 from §10 run as-is (they hit the broker directly, no auth involved). Test 5 runs partially: synth/live canary assertions + the direct DB role check; scope-escalation assertions are deferred to MVP. Tests 6 and 11 are MVP-only.
