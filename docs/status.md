...
| Container runtime selection | **partial** | `server.runtime` picks `docker` or `podman`, and every subcommand is argv-compatible. Podman is selectable and checkable but unverified: rootless networking and volume labelling are untested here. |
| Guided `warehousd init` | **real** | Picks the container engine, the local database and the production database from the registries, then checks each CLI those answers need and offers to install it through whichever package manager this machine has. Never installs without consent, never runs `sudo`, never automates a browser login. `--manual` skips all of it. |
...
| Multi-tenancy (`org_id`) | *partial* | Every grant, audit event and document carries an org, isolated by a view predicate and RLS. A single implicit org is created at bootstrap; there is no UI for creating or switching orgs yet, and no step that resolves a caller's org from their session or IdP claim. |
